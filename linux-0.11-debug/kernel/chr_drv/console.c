...
#define ORIG_X			(*(unsigned char *)0x90000)							// 光标行号
#define ORIG_Y			(*(unsigned char *)0x90001)							// 光标列号
...
#define VIDEO_TYPE_EGAC		0x21	/* EGA/VGA in Color Mode	*/			// EGA/VGA
...
static unsigned long	pos;		/* pos 为当前光标位置所对应的内存指针 */
static unsigned long	x,y;		/* x：光标所在行数；y：光标所在列数 */
...
void con_init(void)
{
	...
	origin	= video_mem_start;
	scr_end	= video_mem_start + video_num_lines * video_size_row; // 屏幕最后一个字符对应的显存地址
	top	= 0;						// 顶行
	bottom	= video_num_lines;		// 尾行
