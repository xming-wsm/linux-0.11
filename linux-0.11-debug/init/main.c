...
{               /* The startup routine assumes (well, ...) this */
...
#endif
  /* 内存初始化：划分内核代码区、缓冲区、主存区，并初始化内存位图 mem_map */
  ...
  /* 
   * Linux将操作系统特权级分为用户态、内核态。该函数的作用是开启用户态，
   * 后续代码会一直处于用户态，除非发生系统调用的中断。
   * 不过，当系统调用中断结束后，会通过中断调用返回指令从内核态回到用户态。
   * 
   * 1. 用户态、内核态的区别是什么？特权等级体现在何处？
   * 2. 如何切换？具体涉及到哪个寄存器、哪个变量？
   * 3. 用户态、内核态是 Linux 自己规定的？还是说和硬件也有关系？
   */
  move_to_user_mode();
